...
describe('Headers', function () {
  it('should init with object/2d array/null', () => {
    expect(() => new HeaderList({})).not.toThrowError()
    expect(() => new HeaderList([])).not.toThrowError()
  })

  it('should fail when init with unidimensional array', () => {
    expect(() => new HeaderList(['test', 'value'])).toThrowError()
  })
  ...
  it('should return isEmpty() equal to true when there is no records', () => {
    expect(new HeaderList({}).isEmpty()).toBeTruthy()
  })
  ...
  it('should contain symbol toStringTag equal to HeaderList', function () {
    expect(new HeaderList({})[Symbol.toStringTag]).toStrictEqual('HeaderList')
  })
})
