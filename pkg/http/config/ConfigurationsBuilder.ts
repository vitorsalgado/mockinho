...
import { Level, Logger } from '../../../internal/log/Logger'
import { HttpServerFactory } from '../HttpServer'
import { Configurations } from './Configurations'

const STUB_FIXTURES_DIR = '__fixtures__'
const STUB_FIXTURES_BODY_FILES_DIR = '__content__'
...
> {
  ...
  protected _stubsDirectory: string = Path.join(process.cwd(), `${STUB_FIXTURES_DIR}`)
  protected _stubsBodyContentDirectory: string = Path.join(
    process.cwd(),
    `${STUB_FIXTURES_DIR}/${STUB_FIXTURES_BODY_FILES_DIR}`
  )
  ...
  stubsDirectory(path: string, bodies?: string): this {
    ...
    this._stubsBodyContentDirectory = bodies
      ? Path.resolve(bodies)
      : Path.join(path, `${STUB_FIXTURES_BODY_FILES_DIR}`)
